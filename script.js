const products=[
['Pixie Cut','Pixie Cut',191,'Natural Black','8 inch','HD Lace','pixie-cut.jpg'],['Pixie Curls','Pixie Curls',222,'Espresso','12 inch','Transparent Lace','pixie-curls.jpg'],['Bone Straight','Bone Straight',253,'Honey Blonde','16 inch','4x4 Closure','bone-straight.jpg'],['Body Wave','Body Wave',283,'Burgundy','20 inch','5x5 Closure','body-wave.jpg'],['Deep Wave','Deep Wave',314,'Pink','24 inch','13x4 Lace Front','deep-wave.jpg'],['Water Wave','Water Wave',344,'Blonde','30 inch','HD Lace','water-wave.jpg'],['Loose Deep Waves','Loose Deep Waves',497,'Burgundy','20 inch','HD Lace','loose-deep-1.jpg'],['Straight Bob','Straight Bob',500,'Pink','24 inch','Transparent Lace','straight-bob.jpg'],['Curly Bob','Curly Bob',245,'Blonde','30 inch','4x4 Closure','curly-bob.jpg'],['Burmese Curly','Burmese Curly',229,'Espresso','12 inch','HD Lace','burmese-curly.jpg'],['Deep Curly','Deep Curly',337,'Pink','24 inch','5x5 Closure','deep-wave-alt.jpg'],['Afro Kinky','Afro Kinky',150,'Blonde','30 inch','13x4 Lace Front','water-wave.jpg'],['Layered Straight','Layered Straight',245,'Natural Black','20 inch','HD Lace',''],['Layered Body Wave','Layered Body Wave',339,'Natural Black','20 inch','Transparent Lace','layered-body-wave.jpg']];
...
const chips=document.querySelector('#chips'); cats.forEach((c,i)=>{let b=document.createElement('button');b.className='chip'+(i===0?' active':'');b.textContent=c;b.onclick=()=>{document.querySelectorAll('.chip').forEach(x=>x.classList.remove('active'));b.classList.add('active');render(c==='All styles'?'':c)};chips.appendChild(b)});
function render(filter=''){const q=(document.querySelector('#search').value||'').toLowerCase();let list=products.filter(p=>(!filter||p[0]===filter)&&(!q||p[0].toLowerCase().includes(q)));document.querySelector('#products').innerHTML=list.map((p,i)=>`<article class="product"><img class="product-img" src="${p[6]||'deep-wave.jpg'}" onerror="this.src='deep-wave.jpg'"><span class="badge">${p[5]}</span><small>${p[5].toUpperCase()}</small><h3>${p[1]}</h3><div class="meta">${p[1]}<br>● ${p[3]} · ${p[4]}</div><div class="price">$${p[2]}</div><select><option>${p[3]}</option></select><select><option>${p[4]}</option></select><button class="add" onclick="addCart(${i},'${filter}')">ADD TO CART</button></article>`).join('')}
...
window.addCart=(i,filter)=>{let p=currentList(filter)[i];cart.push(p);localStorage.cart=JSON.stringify(cart);update()};
